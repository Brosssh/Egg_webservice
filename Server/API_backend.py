...
def get_leaderboard(mongo, element, n, top_n):
    api_logger.info('Called get_leaderboard with parameter: '+str(element) if element is not None else "" +" : "+str(n) if n is not None else ""+" : "+str(top_n)if top_n is not None else "")
