            ...
            if count==0:
                if "HENERPRISE:EPIC" in el["backup"]["artifactsDb"]["shipsCountArchiveAR"]:
                    ...
                    if count_exthens > max_exthens:
                        ...
                        result={"user_name":user_name,"report_date":str(datetime.date.today()),"count_exthens":max_exthens}
        except Exception as e:
...
print("Fetching all the backups, this may take a while...")
file_list = list(mongo.get_users_files())
print("Backups fetched, starting calculations")

final_dict_report["leg_seen"]=legendary_seen(file_list)
final_dict_report["legendary_players"]=get_dict_legendary_players(file_list)
final_dict_report["zlc_record"]=get_zlc_record(file_list,old_file["report"]["zlc_record"])
final_dict_report["number_total_users"]=len(file_list)
